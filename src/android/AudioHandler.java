...
/**
 * This class called by CordovaActivity to play and record audio. The file can be local or over a
 * network using http.
 *
 * Audio formats supported (tested): .mp3, .wav
 *
 * Local audio files must reside in one of two places: android_asset: file name must start with
 * /android_asset/sound.mp3 sdcard: file name is just sound.mp3
 */
public class AudioHandler extends CordovaPlugin {
    ...
    public static String[] permissions = {Manifest.permission.RECORD_AUDIO, // 録音
            Manifest.permission.WRITE_EXTERNAL_STORAGE // 外部ストレージの使用
    };
    ...
    public void setAudioOutputDevice(int output) {
        ...
        AudioManager audiMgr =
                (AudioManager) this.cordova.getActivity().getSystemService(Context.AUDIO_SERVICE);
        if (output == 2) {
            audiMgr.setRouting(AudioManager.MODE_NORMAL, AudioManager.ROUTE_SPEAKER,
                    AudioManager.ROUTE_ALL);
        } else if (output == 1) {
            audiMgr.setRouting(AudioManager.MODE_NORMAL, AudioManager.ROUTE_EARPIECE,
                    AudioManager.ROUTE_ALL);
        } else {
    ...
    public void onRequestPermissionResult(int requestCode, String[] permissions, int[] grantResults)
        ...
        for (int r : grantResults) {
            if (r == PackageManager.PERMISSION_DENIED) {
                this.messageChannel.sendPluginResult(
